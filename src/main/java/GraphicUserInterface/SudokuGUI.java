...
public class SudokuGUI {
    ...
    public SudokuGUI(SudokuGameImpl peer, int peerID, User user) throws IOException, ParseException {
        this.peer=peer;
        this.peerID=peerID;
        this.user=user;
        for(int row=0; row < 9 ; row++){
            for(int column=0; column < 9 ; column++){
                ...
                textField[row][column].addActionListener(new ActionListener() {

                    public void actionPerformed(ActionEvent e) {
                        ((JTextField) e.getSource()).setBackground(new Color(229,247,255));
                    }
                });
                ...
                if(room_active.size()==0){
                 ...
                 /*
                    if(selectedValue!= null){
                        room=selectedValue.toString;
                        if(room==stanzagiaesistente){
                            recuperailsudokudaquellastanzaestampalo
                            for (int row = 0; row < 9; row++) {
                                for (int column = 0; column < 9; column++) {
...
//                        }
        ...
        final JButton buttonSudoku = new JButton("Get a Sudoku");
